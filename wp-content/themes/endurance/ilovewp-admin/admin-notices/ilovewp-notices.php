...
class endurance_notice {
	...
	private function is_dismiss_notice() {
		return apply_filters( 'endurance_' . $this->name . '_notice_dismiss', true );
	}
	...
	public function get_notices() {
		...
		$ilovewp_theme_admin_notices = get_option( 'endurance_admin_notices' );
	...
	public function hide_notices() {
		...
		if ( isset( $_GET['endurance-hide-notice'] ) && isset( $_GET['_endurance_notice_nonce'] ) ) {
			
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_endurance_notice_nonce'] ), 'endurance_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'endurance' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'endurance' ) );
			}

			if ( $_GET['endurance-hide-notice'] ) {
				
				$hide_notice_id = sanitize_text_field( wp_unslash( $_GET['endurance-hide-notice'] ) );
				...
				if ( isset($run_update) ) {
					update_option( 'endurance_admin_notices', $theme_admin_notices );
				}
